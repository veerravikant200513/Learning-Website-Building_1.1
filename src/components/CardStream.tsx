...
// Using window.THREE from script tag since npm install three is not available
declare global {
    interface Window {
        THREE: any;
...
export default function CardStream() {
    ...
    useEffect(() => {
        ...
        let cardStreamInst: any;
        let particleSystemInst: any;
        let particleScannerInst: any;
        ...
        class CardStreamController {
            ...
            constructor(containerEl: HTMLElement, cardLineEl: HTMLElement, speedEl: HTMLElement | null) {
                this.container = containerEl;
                this.cardLine = cardLineEl;
                this.speedIndicator = speedEl;
            ...
            setupEventListeners() {
                ...
                this.cardLine.addEventListener("touchstart", (e) => this.startDrag(e.touches[0] as any), { passive: false });
                document.addEventListener("touchmove", (e) => this.onDrag(e.touches[0] as any), { passive: false });
            ...
            createCardWrapper(index: number) {
                ...
                normalCard.className = "card tc-card-normal";
                ...
                asciiCard.className = "card tc-card-ascii";
            ...
            updateCardClipping() {
                ...
                document.querySelectorAll(".card-wrapper").forEach((wrapper) => {
                    ...
                    const normalCard = wrapper.querySelector(".tc-card-normal") as HTMLElement;
                    const asciiCard = wrapper.querySelector(".tc-card-ascii") as HTMLElement;
        ...
        setTimeout(() => {
            if (containerRef.current && cardLineRef.current) {
                cardStreamInst = new CardStreamController(containerRef.current, cardLineRef.current, speedIndicatorRef.current);
                particleSystemInst = new ParticleSystem(document.getElementById("particleCanvas") as HTMLCanvasElement);
                particleScannerInst = new ParticleScanner(document.getElementById("scannerCanvas") as HTMLCanvasElement);

                window.setScannerScanning = (active: boolean) => {
                    if (particleScannerInst) {
                        particleScannerInst.setScanningActive(active);
                    }
                };

                // @ts-ignore
                window.cardStreamControllerInstance = cardStreamInst;
            }
        }, 100);

        return () => {
            if (cardStreamInst) cardStreamInst.destroy();
            if (particleSystemInst) particleSystemInst.destroy();
            if (particleScannerInst) particleScannerInst.destroy();
            delete window.setScannerScanning;
        };
    }, []);

    return (
        <div className="w-full relative mt-32 mb-10 overflow-hidden"
            style={{ minHeight: '600px', backgroundColor: '#000000', fontFamily: 'Arial, sans-serif' }}>

            <div className="w-full relative text-center pt-10 pb-20 z-10 bg-gradient-to-b from-[#000000] to-transparent">
                <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif text-white/90">
                    Stop burning cards on useless content
                </h2>
            </div>
            ...
            <div className="tc-container" ref={containerRef} id="cardStream">
                ...
                <canvas id="scannerCanvas" className="tc-scannerCanvas"></canvas>

                <div className="tc-scanner"></div>

                <div className="tc-card-stream">
                    <div className="tc-card-line" ref={cardLineRef} id="cardLine"></div>
                </div>
            </div>
        </div>
    );
