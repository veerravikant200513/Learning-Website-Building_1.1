...
const Background = () => (
  <div className="fixed inset-0 -z-10 overflow-hidden bg-[var(--color-background)]">
    ...
    <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] bg-purple-600/10 rounded-full blur-[150px] animate-float-delayed" />
    <div className="absolute top-[20%] right-[10%] w-[40%] h-[40%] bg-indigo-600/5 rounded-full blur-[100px] animate-float-slow" />
      ...
      <div className="glass px-2 py-2 rounded-full flex items-center gap-1">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 relative cursor-pointer ${activePage === tab ? 'text-white' : 'text-white/60 hover:text-white/90'
              }`}
          >
            {activePage === tab && (
              <motion.div
...
const Philosophy = () => (
  ...
  >
    ...
    <div className="space-y-8 text-xl md:text-2xl leading-relaxed text-white/90 font-light">
      <p>
          ...
          {milestones.map((m, i) => (
            ...
            >
              <div className="flex-1 text-center md:text-left">
                <div className={`glass p-8 rounded-3xl hover:bg-white/10 transition-colors duration-300 cursor-pointer ${i % 2 === 0 ? 'md:text-right' : 'md:text-left'}`}>
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {goals.map((g, i) => (
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ delay: i * 0.1 }}
            className="glass p-8 rounded-[2rem] flex flex-col items-center text-center group hover:bg-white/10 transition-colors duration-300 cursor-pointer"
          >
            <div className="mb-6 p-4 rounded-2xl bg-white/5 text-blue-400 group-hover:text-blue-300 transition-colors duration-300">
      ...
      <div className="mt-20 relative rounded-[3rem] overflow-hidden aspect-[21/9] glass">
        ...
        <div className="absolute inset-0 flex items-center justify-center p-12 text-center pointer-events-none">
          <p className="text-3xl md:text-5xl font-serif text-white/90">
  ...
  return (
    ...
    >
      <div className="text-center mb-16">
        ...
        <p className="text-white/70 text-lg">I'm always open to new ideas and collaborations.</p>
      </div>
            ...
            <div className="flex gap-4">
              {[
                ...
                <a
                  ...
                  className="w-16 h-16 rounded-2xl glass flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 transition-colors duration-300 cursor-pointer"
